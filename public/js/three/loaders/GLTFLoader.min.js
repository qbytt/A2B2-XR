...
function createDefaultMaterial(cache){if(cache.DefaultMaterial===undefined){cache.DefaultMaterial=new THREE.MeshStandardMaterial({color:0xFFFFFF,emissive:0x000000,metalness:1,roughness:1,transparent:!1,depthTest:!0,side:THREE.FrontSide})}
...
function addMorphTargets(geometry,targets,parser){let hasMorphPosition=!1;let hasMorphNormal=!1;for(let i=0,il=targets.length;i<il;i ++){const target=targets[i];if(target.POSITION!==undefined)hasMorphPosition=!0;if(target.NORMAL!==undefined)hasMorphNormal=!0;if(hasMorphPosition&&hasMorphNormal)break}
...
const URL=self.URL||self.webkitURL;let sourceURI=source.uri||'';let isObjectURL=!1;let hasAlpha=!0;const isJPEG=sourceURI.search(/\.jpe?g($|\?)/i)>0||sourceURI.search(/^data\:image\/jpeg/)===0;if(source.mimeType==='image/jpeg'||isJPEG)hasAlpha=!1;if(source.bufferView!==undefined){sourceURI=parser.getDependency('bufferView',source.bufferView).then(function(bufferView){if(source.mimeType==='image/png'){const colorType=new DataView(bufferView,25,1).getUint8(0,!1);hasAlpha=colorType===6||colorType===4||colorType===3}
isObjectURL=!0;const blob=new Blob([bufferView],{type:source.mimeType});sourceURI=URL.createObjectURL(blob);return sourceURI})}else if(source.uri===undefined){throw new Error('THREE.GLTFLoader: Image '+textureIndex+' is missing URI and bufferView')}
...
loader.load(resolveURL(sourceURI,options.path),onLoad,undefined,reject)})}).then(function(texture){if(isObjectURL===!0){URL.revokeObjectURL(sourceURI)}
...
const targetName=node.name?node.name:node.uuid;const interpolation=sampler.interpolation!==undefined?INTERPOLATION[sampler.interpolation]:THREE.InterpolateLinear;const targetNames=[];if(PATH_PROPERTIES[target.path]===PATH_PROPERTIES.weights){node.traverse(function(object){if(object.isMesh===!0&&object.morphTargetInfluences){targetNames.push(object.name?object.name:object.uuid)}})}else{targetNames.push(targetName)}
